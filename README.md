...
- Cost control is key to successful competition in the airline industry. This is an issue to find out how to schedule United Airlines staffs to provide necessary services to customers and minimize costs at the airport. The following table lists the cost per shift and the minimum number of staff required to be on duty for each two-hour interval.
...
- The details such as how to obtain the minimum number of staffs required for one day and how to handle weekly off days are not explained (or published) in the book. Using the algorithms provided by our engine, we can conclude that the minimum number of staffs required per day is 176. The optimal number of people required to meet the constraints of each staff having one day-off per week and each staff having two day-off per week is 206 and 247 respectively.
   ...
   - Here is the  Gantt-chart of the weekly schedule for 247 staffs. Excel file (Personnel_Schedule_247-7-12.xlsx) can be found  In the "Examples/Airport Staff Schedule" folder.
     ![image](https://github.com/weiran-aitech/shift_schedule/assets/84350533/468c3c8d-eba1-48e2-a76e-923406f21a0a)
In this schedule, there are 244 staffs who have two day-off a week and 3 staffs who have three day-off a week. If 246 staffs are arranged to participate in the schedule, 2 staffs must work 6 days a week (only one day-off). Considering the need to arrange staffs to fill vacancies in case of emergencies, 247 staffs are the best number of staffs to meet the constraint of two day-off per week for each staff. In addition to this primary constraint, the schedule also ensures that each person's shifts are at least 12 hours apart from the previous day's and the next day's. Any staff working the shift 5 (from 10:00 P.M to 6:00 A.M) one day is scheduled to have day-off the next day. Note that, more constraints can be added to further improve the schedule.
